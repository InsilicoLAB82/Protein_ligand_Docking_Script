...
import numpy as np
import seaborn as sns  # For kernel density estimation (KDE)
...
print("\n" + "═" * 80)
print("               AUTODOCK VINA DOCKING OUTPUT")
print("═" * 80 + "\n")
...
# Plot results: Combine RMSD vs Affinity scatter plot and RMSD distribution with KDE curve
if affinities:
    fig, axs = plt.subplots(1, 2, figsize=(14,6))  # 1 row, 2 columns

    # Scatter plot for RMSD vs Affinity
    sc = axs[0].scatter(rmsd_list, affinities, c=affinities, cmap='coolwarm_r', s=120, edgecolors='k')
    axs[0].set_xlabel('RMSD from Crystal Pose (Å)', fontsize=14)
    axs[0].set_ylabel('Binding Affinity (kcal/mol)', fontsize=14)
    axs[0].set_title(f'{scoring.upper()} Docking – Exhaustiveness {exhaustiveness}', fontsize=16)
    best_idx = np.argmin(affinities)
    axs[0].scatter(rmsd_list[best_idx], affinities[best_idx], c='red', s=400, marker='*',
                   label=f'Best: {affinities[best_idx]:.2f} kcal/mol @ {rmsd_list[best_idx]:.2f} Å')
    axs[0].legend()
    axs[0].grid(alpha=0.3)

    # RMSD distribution plot with KDE curve
    sns.kdeplot(rmsd_list, ax=axs[1], fill=True, color='skyblue', linewidth=2)  # KDE for smooth curve
    axs[1].hist(rmsd_list, bins=20, color='lightgray', edgecolor='black', alpha=0.5)  # Histogram
    axs[1].set_xlabel('RMSD (Å)', fontsize=14)
    axs[1].set_ylabel('Frequency', fontsize=14)
    axs[1].set_title('RMSD Distribution with KDE Curve', fontsize=16)

    # Add watermark text to the plot
    plt.figtext(0.5, 0.01, 'Generated by InsilicoLAB insilicolab82@gmail.com', ha='center', fontsize=12, color='gray', alpha=0.5)

    # Adjust layout and save both plots in a single file
    plt.tight_layout()
